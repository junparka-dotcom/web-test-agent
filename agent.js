...
(async () => {
  ...
  const findings = []; // report_finding 호출 내역 누적 (로그아웃 강제 종료 시 자동 요약에 사용)
  ...
  for (let step = 1; step <= MAX_STEPS; step++) {
    ...
    // 이번 턴에 온 tool 호출을 전부 가져옴 (하나일 수도, 여러 개일 수도 있음)
    ...
    const toolResults = []; // 이번 턴의 모든 tool_use에 대해 반드시 하나씩 채워야 함
    let shouldEnd = false;
    ...
    for (const toolUse of toolUses) {
      ...
      // 이 tool_use에 대한 결과를 반드시 push (빠뜨리면 아까 같은 에러 발생)
    ...
    if (shouldEnd) {
      if (endReason === 'logout') {
        const summaryLines = findings.map(f => `${f.status === 'ok' ? '✓' : '✗'} ${f.target}: ${f.detail}`).join('\n');
